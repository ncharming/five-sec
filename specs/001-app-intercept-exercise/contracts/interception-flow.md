...
| 区域 | 内容 | 状态依赖 |
|------|------|----------|
| 标题 | "是否打开 {应用名}？"（应用名使用友好名称，如"小红书"；用户可展开查看包名） | 始终 |
| 锻炼引导 | 提肛引导文案 + 大号倒计时数字 5→0 | 倒计时进行中 |
| "打开"按钮 | **5秒倒计时期间：置灰且禁用**，视觉上传达不可用状态，避免用户误认为功能故障；倒计时结束：正常可用 | `BLOCKING`→置灰且禁用；`CHOICE_UNLOCKED`→正常可用 |
| "取消"按钮 | 同上 | 同上 |

**按钮状态细节**（FR-006 澄清）：
- `BLOCKING` 状态：按钮显示为置灰（视觉上灰色），且不响应点击事件（功能上禁用）
- `CHOICE_UNLOCKED` 状态：按钮从置灰禁用变为正常可用状态，用户可以点击并产生预期响应
