...
五秒（five-sec）是纯离线的 Android 自律工具：无障碍服务检测到目标应用（默认抖音/小红书/B站）进入前台时，弹全屏 `TYPE_ACCESSIBILITY_OVERLAY` 覆盖层强制 5 秒提肛倒计时，结束后用户选「打开」（写抑制窗口放行）或「取消」（回桌面），每次拦截写入本地事件流水供统计页实时聚合；拦截页的核心提醒位是**今日待办卡片**（specs/009 起为唯一缓冲内容，把减速带时机变成每日任务的提醒位；提示语功能已整体退役落地——覆盖层不再展示提示语行、「提示语」Tab 已移除，空态由待办卡常驻补位：无启用条目=引导添加、有启用但今日不轮到=告知）；无账号、无网络、无后端，数据全在设备本地。
...
data/           db/（Room：AppDatabase v9 + DAO，hints 物理表保留不声明）、datastore/（SettingsDataStore）、repository/、seed/
...
di/             AppModule：唯一的 Hilt @Module（DB、DAO、迁移链 1→9、TimeProvider、应用级 CoroutineScope）
