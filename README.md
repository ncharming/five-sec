...
打开抖音、小红书、B站等沉浸式应用前，先弹窗拦截，引导一次 5 秒提肛（凯格尔）锻炼，并展示你的**今日待办**，再问你是否真的要打开——给无意识的刷手机加一道"减速带"：把"要不要刷"换成"今天还有什么没做"，顺带完成一次日常微锻炼。

## 工作原理

1. 通过 **无障碍服务（AccessibilityService）** 检测目标应用进入前台。
2. 命中后直接弹出全屏拦截覆盖层（`TYPE_ACCESSIBILITY_OVERLAY` 窗口，非 Activity——规避 OEM 对后台启动界面的静默拦截）。
3. 拦截页强制 **5 秒提肛倒计时**，期间"打开/取消"按钮锁定；页面展示**今日待办**卡片——拦截缓冲层的唯一内容（[specs/009](specs/009-retire-hints/spec.md) 起提示语退役）。
4. 倒计时结束后可选择：
   - **打开**：在 5 秒抑制窗口内重新启动目标应用（避免二次拦截）。
   - **取消**：留在桌面。
5. 所有数据本地存储（Room + DataStore），**无需账号、无需联网**。

## 每日待办（specs/005-daily-todos）—— 核心功能

待办是五秒的核心功能、拦截缓冲层的**唯一**内容（[specs/009](specs/009-retire-hints/spec.md) 起提示语退役）：5 秒减速带里展示的就是你自己的今日清单。

底部三个 Tab：**待办 / 统计 / 设置**（specs/009 移除「提示语」Tab 后的形态；原「拦截」Tab 更名「设置」并居最右，内容不变），默认落在「待办」。
...
- **拦截时机提醒**：任一目标应用触发拦截时，覆盖层会用一块紧凑卡片展示「今日待办 2/5」与未完成条目（排序后只展示前 3 条：第一层按类型 仅今天→每N天→每周几→每天，第二层同类型内创建时间倒序；每条截前 12 字，卡片内容与页面其余部分统一居中）；全部完成显示"今日待办已全部完成 ✓"；没有「轮到且启用」条目时显示空态文案（无启用条目=「还没有今日待办 · 打开「五秒」添加」引导去添加、有启用但今日不轮到=「今天没有轮到的待办」），卡片常驻不隐藏（specs/009 空态二分）。卡片只读，勾选回待办页完成。倒计时为「请先思考 N 秒」文字行（22sp 品牌绿，无大数字块），结束变「✓ 请选择」。
- 待办**不产生任何通知**；提醒只发生在拦截弹出的那一刻；完成情况进统计页（specs/008「任务完成统计」）。

## 提示语功能（已退役）

提示语与待办服务于同一目标——在减速带里引导用户做更有意义的事——功能重复，而泛化文案的价值弱于个人化的待办清单（待办还有完成统计闭环）。**已整体退役**（[specs/009-retire-hints](specs/009-retire-hints/spec.md)，2026-09-24 拍板并落地）：

- 待办成为拦截页**唯一**缓冲内容：覆盖层移除提示语行，空态由待办卡补位（引导添加 / 告知今天没有轮到的待办）；
- 「提示语」Tab 与管理页移除，底部导航变为 **待办 / 统计 / 设置** 三项；
- 自定义提示语**数据不删**：`hints` 物理表只读保留（Room v9 空迁移，零破坏性操作）、不做任何清理；循环游标与内置开关停用后残留不清理。

> 以下为退役**前**的历史行为，仅供考古（specs/004-custom-hints 引入，specs/005-daily-todos 改为循环）：
...
2. 默认已内置抖音、小红书、B站三个目标应用；在「设置」Tab 可管理无障碍状态（置顶状态卡：未开启时淡红警示、点击一键开启）、拦截总开关（应用清单卡卡头，统一控制清单内应用；关闭时应用行降透明度）与目标应用增删。
3. 在「待办」Tab 添加你的每日任务，之后每次拦截都会顺带提醒你今天还剩什么没做。
4. 点击桌面抖音图标即可触发拦截页。

## 统计与数据留存（specs/008 起双块结构）

- 统计主页为**今日概览**（specs/008 落地后经用户拍板视觉重构为一屏两域双 Hero 卡）：**今日任务**卡——`完成/轮到` 大字 + 品牌绿完成进度条 + 百分比，全部完成显示「✓ 全部完成」胶囊，过期数 >0 红色警示（任务 = 今天轮到且启用，与拦截卡片 D/T 同口径 / 完成 = 其中已勾选 / 过期 = 一次性过期待办数，仅今日出现）；**今日拦截**卡——拦截次数与连续完成天数双 Hero（发丝线分隔），底部小字取消/打开；两张卡**整卡可点**分别进入**任务完成统计 ›** 与**应用拦截统计 ›** 二级页（主页不再放独立入口行）；
...
- [009-retire-hints](specs/009-retire-hints/spec.md)：提示语功能退役——待办成为拦截页唯一缓冲内容（空态引导 / 提示语 Tab 移除 / 存量数据零破坏）（[plan](specs/009-retire-hints/plan.md)、[data-model](specs/009-retire-hints/data-model.md)、[quickstart](specs/009-retire-hints/quickstart.md)）
