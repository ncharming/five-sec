...
3. 拦截页强制 **5 秒提肛倒计时**，期间"打开/取消"按钮锁定；页面同时展示一条提示语（内置+自定义按顺序**循环**轮换，**规划退役中**，见 [specs/009](specs/009-retire-hints/spec.md)）和**今日待办**卡片。
4. 倒计时结束后可选择：
...
待办是五秒的核心功能与拦截缓冲层的内容主体：5 秒减速带里展示的就是你自己的今日清单；[specs/009](specs/009-retire-hints/spec.md) 起规划升格为**唯一**提醒位（提示语退役）。

底部四个 Tab：**待办 / 提示语 / 统计 / 设置**（原「拦截」Tab 更名「设置」并移到最右，内容不变；提示语 Tab 规划移除后为三项），默认落在「待办」。
...
- **拦截时机提醒**：任一目标应用触发拦截时，覆盖层会用一块紧凑卡片展示「今日待办 2/5」与未完成条目（排序后只展示前 3 条：第一层按类型 仅今天→每N天→每周几→每天，第二层同类型内创建时间倒序；每条截前 12 字，卡片内容与页面其余部分统一居中）；全部完成显示"今日待办已全部完成 ✓"；没有启用待办时整块隐藏（specs/009 规划：改为显示空态——无启用条目引导去添加、有启用但今日不轮到则告知"今天没有轮到的待办"）。卡片只读，勾选回待办页完成。倒计时为「请先思考 N 秒」文字行（22sp 品牌绿，无大数字块），结束变「✓ 请选择」。
- 待办**不产生任何通知**；提醒只发生在拦截弹出的那一刻；完成情况进统计页（specs/008「任务完成统计」）。

## 提示语功能（已规划退役）

提示语与待办服务于同一目标——在减速带里引导用户做更有意义的事——功能重复，而泛化文案的价值弱于个人化的待办清单（待办还有完成统计闭环）。**已拍板整体退役**（[specs/009-retire-hints](specs/009-retire-hints/spec.md)，2026-09-24）：

- 待办成为拦截页**唯一**缓冲内容：覆盖层移除提示语行，空态由待办卡补位（引导添加 / 告知今天没有轮到的待办）；
- 「提示语」Tab 与管理页移除，底部导航变为 **待办 / 统计 / 设置** 三项；
- 自定义提示语**数据不删**：只读保留、不做破坏性迁移；循环游标与内置开关停用后残留不清理。

> 以下为退役落地**前**的现行行为，仅供过渡期参考（specs/004-custom-hints 引入，specs/005-daily-todos 改为循环）：
...
- [009-retire-hints](specs/009-retire-hints/spec.md)：提示语功能退役——待办成为拦截页唯一缓冲内容（空态引导 / 提示语 Tab 移除 / 存量数据零破坏）
