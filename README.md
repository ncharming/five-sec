...
底部四个 Tab：**待办 / 提示语 / 统计 / 设置**（原「拦截」Tab 更名「设置」并移到最右，内容不变），默认落在「待办」。

- **待办页**（specs/007 起分两区）：「**今日待办**」卡维护一切未过期条目——新增、修改、删除、启用/停用、当天勾选，按重复规则自动回到未完成（惰性求值，无需手动重置）；「**过期待办 (n)**」卡收纳一次性条目的失败存量（按有效期日升序，最早失败在前），可「改为今天」重试或删除，**无迟到补勾**；无过期时第二卡不显示。每行标题下副行显示纯日期（今日区=创建日，v7 前老条目显示「—」；过期区=有效期日）。上限 20 条（两区共享）、每条 ≤200 字（列表单行省略展示，点条目弹窗看全文；拦截卡片每条仅展示前 12 字）。**重复规则四选一**：每天（默认）/ 每周几（多选周一~周日，仅选中日轮到）/ 每N天（2–365，滚动节奏：从未完成立即可做、完成后隔 N 天再次出现、到期持续提醒直到完成；编辑时下方实时显示「下次执行」日期，锚点=最近完成日、从未完成为「今天」）/ **单次**（原「仅今天」：一次性，只在有效期日当天轮到，当天完成跨日自动清理，当天没完成跨日进过期分类）；选项分段按钮不显示选中 ✓（选中态由底色表达）；不轮到的日子条目灰显「今天不用做」不可勾、不计进度、不上拦截卡片；切换规则不清完成状态、不改创建时间；重复类错过当天不进过期分类（顺延到下次轮到）。
- **拦截时机提醒**：任一目标应用触发拦截时，覆盖层会用一块紧凑卡片展示「今日待办 2/5」与未完成条目（排序后只展示前 3 条：第一层按类型 仅今天→每N天→每周几→每天，第二层同类型内创建时间倒序；每条截前 12 字，卡片内左对齐）；全部完成显示"今日待办已全部完成 ✓"；没有启用待办时整块隐藏。卡片只读，勾选回待办页完成。倒计时为「请先思考 N 秒」文字行（22sp 品牌绿，无大数字块），结束变「✓ 请选择」。
- 待办**不产生任何通知**；提醒只发生在拦截弹出的那一刻；完成情况进统计页（specs/008「任务完成统计」）。
...
2. 默认已内置抖音、小红书、B站三个目标应用；在「设置」Tab 可管理总开关、无障碍状态与目标应用增删。
3. 在「待办」Tab 添加你的每日任务，之后每次拦截都会顺带提醒你今天还剩什么没做。
4. 点击桌面抖音图标即可触发拦截页。

## 统计与数据留存（specs/008 起双块结构）

- 统计主页为**今日概览**：今日拦截/取消/打开三列卡与连续完成天数卡（锚点），加**今日任务三列卡**（任务 = 今天轮到且启用，与拦截卡片 D/T 同口径 / 完成 = 其中已勾选 / 过期 = 一次性过期待办数）；下方两张入口卡：**应用拦截统计 ›** 与**任务完成统计 ›**（specs/008）；
- 当天没有任何记录时统计页**照常渲染**，各项以 0 值展示（查询始终执行，"零"是有效数据而非空态缺失）；
- **应用拦截统计**二级页：按 **日 / 周 / 月 / 年** 自然周期切换（周从周一起算）；周可筛本周/上周，月可筛当年 1 月至今，年可筛最早记录年份至今，不提供未来周期；每个清单应用一张品牌色卡片展示所选周期内的拦截/打开/取消次数；
- **任务完成统计**二级页：同款档位与周期筛选（与拦截二级页共享选择）；顶部为周期内总完成次数，下面按待办条目卡展示「完成 N 次」（次数降序）；**历史只展完成数**——应做数与过期数是当下才可算的真值，历史不可知故不伪造；已删除条目按勾选当时的文本快照展示；
- 待办每次勾选即写入 `todo_completions` 完成事件（当天同条唯一：勾选落一行、取消即删、重勾不重复），历史统计由事件流水实时聚合，自升级部署日起积累（更早的完成无从追溯）；
...
- [008-todo-stats](specs/008-todo-stats/spec.md)：任务完成统计 + 统计页双块重构（完成事件表 / 今日任务三数 / 两个历史二级页）（[plan](specs/008-todo-stats/plan.md)、[data-model](specs/008-todo-stats/data-model.md)、[quickstart](specs/008-todo-stats/quickstart.md)）
