...
- **拦截时机提醒**：任一目标应用触发拦截时，覆盖层会用一块紧凑卡片展示「今日待办 2/5」与未完成条目（最多 5 条，每条截前 12 字，卡片内左对齐）；全部完成显示"今日待办已全部完成 ✓"；没有启用待办时整块隐藏。卡片只读，勾选回待办页完成。倒计时为「请先思考 N 秒」文字行（22sp 品牌绿，无大数字块），结束变「✓ 请选择」。
